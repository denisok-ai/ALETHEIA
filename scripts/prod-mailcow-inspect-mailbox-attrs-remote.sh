...
docker exec "$M" mysql -u"$DBUSER" -p"$DBPASS" "$DBNAME" -e \
  "SELECT domain, active, defquota, maxquota, quota FROM domain WHERE domain='avaterra.pro'\\G"
docker exec "$M" mysql -u"$DBUSER" -p"$DBPASS" "$DBNAME" -e \
  "SELECT template, attributes FROM templates WHERE type='mailbox' ORDER BY id LIMIT 3\\G"
REMOTE
