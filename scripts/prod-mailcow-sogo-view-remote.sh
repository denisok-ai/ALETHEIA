...
docker exec "$M" mysql -u"$DBUSER" -p"$DBPASS" "$DBNAME" -e "SHOW CREATE TABLE _sogo_static_view\G"
docker exec "$M" mysql -u"$DBUSER" -p"$DBPASS" "$DBNAME" -e "SELECT * FROM _sogo_static_view LIMIT 10;"
REMOTE
