...
# RESET_AND_SEED=1 — на сервере prisma migrate reset + seed (все данные БД удаляются). Только для стенда/пустого прода.
ssh -i "$KEY" -o IdentitiesOnly=yes -o ConnectTimeout=30 "${USER}@${HOST}" \
  "REMOTE_DIR=$REMOTE_DIR_Q GIT_BRANCH=${GIT_BRANCH:-main} SKIP_NGINX_CACHE=${SKIP_NGINX_CACHE:-0} RESET_AND_SEED=${RESET_AND_SEED:-0} bash -s" <<'REMOTE'
set -euo pipefail
