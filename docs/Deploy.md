...
### Проверка после выката на прод (smoke)

Выполнить на **боевом домене** (например https://avaterra.pro) после `deploy-pull` / systemd restart / первого деплоя Vercel:

1. **API:** `GET /api/health` — `200`, в теле `ok: true`, `database: "ok"`; при необходимости сверить `version` / `commit` с ожидаемой сборкой. Если `503` — проверить `DATABASE_URL`, права на файл SQLite, миграции Prisma.
2. **Публичный сайт:** главная, `/about`, `/faq`, `/contacts`, `/blog`, страница курса `/course/navyki-myshechnogo-testirovaniya` (старый `/courses` редиректит на неё) — без 5xx; кастомная **404** на несуществующем пути.
