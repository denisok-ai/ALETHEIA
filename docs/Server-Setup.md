...
> **Неактуально для текущего продуктива avaterra.pro.** Активная схема: каталог **`/opt/ALETHEIA`**, **systemd** `aletheia.service`, порядок обновления и диагностика — [Production-Server.md](Production-Server.md). Скрипт проверки без изменений на диске: [`scripts/prod-diagnostics.sh`](../scripts/prod-diagnostics.sh).
>
> Этот документ сохранён как **историческая / альтернативная** инструкция (путь `/var/www/…`, PM2). Не смешивать с рабочим продом: второй клон приводит к «двум базам» и рассинхрону витрины/админки.

---

Если папки `/var/www/AVATERRA` ещё нет — выполните эти шаги **один раз** на сервере по SSH *(legacy-сценарий)*.
