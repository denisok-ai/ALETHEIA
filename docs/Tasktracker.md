...
**Версия продукта (package.json):** 3.5.2 — см. CHANGELOG [3.5.2] (2026-05-04): целостность доменных ящиков (Mailcow + Prisma); герой — коллаж слева без обрезки кадра.  
...
| Деплой на прод | Критический | В процессе | Чек-лист: [Deploy.md — раздел «Проверка после выката на прод»](Deploy.md). Закрыть статус «Завершена» после smoke на домене и стабильного `/api/health`. Vercel или VPS: [Production-Server.md](Production-Server.md) |
