...
- **Когда подключим новую кассу:** интеграция создания платежа/webhook/чеков 54-ФЗ/возвратов, затем переключить «Приём оплаты» обратно на онлайн. Заказы pending из заявок можно закрывать вручную после оплаты (через выдачу доступа в админке — Order.userId проставится вместе с зачислением).
