/**
 * Варианты классов кнопки (cva) — без «use client», чтобы безопасно вызывать из Server Components.
 * Компонент {@link Button} реэкспортирует `buttonVariants` из `@/components/ui/button` для клиента.
 */
