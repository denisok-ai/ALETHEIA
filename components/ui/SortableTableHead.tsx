  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={handleClick}
        className="inline-flex items-center gap-1.5 text-left font-semibold text-[var(--portal-text)] hover:text-[var(--portal-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--portal-accent)] focus:ring-offset-1 rounded px-1 -mx-1"
      >
        {children}
        {isActive ? (
          currentSortDir === 'asc' ? (
            <ChevronUp className="h-4 w-4 shrink-0 text-[var(--portal-accent)]" aria-hidden />
          ) : (
            <ChevronDown className="h-4 w-4 shrink-0 text-[var(--portal-accent)]" aria-hidden />
          )
        ) : (
