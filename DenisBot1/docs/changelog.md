...
## [2026-08-24] - Лид-воронка выключена: трафик уходит в бота портала

### Изменено
- Воронка бота отключена флагом `FUNNEL_ENABLED` (по умолчанию `false`). `/start`, кнопки воронки и свободные сообщения от не-админов теперь отвечают редиректом в бота портала — кнопка «Перейти в бот школы» на `https://t.me/<PORTAL_BOT_USERNAME>` (по умолчанию `AvaterraProBot`).
- Лид-события (`lead_funnels`, `lead_events`) при выключенной воронке не пишутся, уведомления админам о лидах не шлются.
- Админские сценарии (контент-план, генерация, публикация, аналитика) не затронуты — роутеры `admin` и `analytics` работают как раньше.

### Причина
- Две параллельные воронки: эта (Python) и воронка портала ALETHEIA (`lib/telegram-bot/funnel.ts`). Лиды этого бота попадали в его собственный Postgres, а не в CRM школы, поэтому менеджеры их не видели. Решение владельца: одна воронка — в боте портала, который заводит лида в CRM.

### Валидация
- `pytest tests/test_funnel_flow.py` — 5 passed (тексты воронки не менялись).
- Проверены значения настроек и ссылка редиректа: `funnel_enabled=False`, кнопка → `https://t.me/AvaterraProBot`.
- **Не выкачено:** хост бота (`/opt/avaterra-bot`) недоступен из рабочей сессии, нужен `deploy/deploy.sh` с `DEPLOY_HOST` со стороны владельца.
