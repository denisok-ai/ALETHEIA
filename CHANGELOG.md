...
### Fixed (2026-09-02) — SCORM: курс ложно ставился на 100% при «зашёл и вышел»

- Плеер помечал урок `completed` из fallback-трекинга на выходе со страницы, если SCORM-событие Initialize не приходило за 10с. Для одно-SCO курса это давало 100% + авто-сертификат без реального прохождения (жалоба: студент «зашёл и вышел» → курс пройден).
- Фикс в `app/portal/student/courses/[courseId]/play/page.tsx`: первый реальный SCORM-коммит выключает fallback; fallback больше НИКОГДА не ставит `completed` (только `incomplete`) — завершение приходит только от самого SCORM. Проверено браузером как тестовый студент.
- **Серверный рубеж (defense-in-depth):** в `app/api/portal/scorm/progress/route.ts` `completed`/`passed` без реального CMI (пустой `cmiData`) понижается до `incomplete` до сохранения — ложное завершение/сертификат невозможны независимо от клиента (баг, подделка). Реальные коммиты несут полный CMI. Проверено live API-тестом: синтетический `completed` без CMI → сохранён `incomplete`, сертификат не выдан.
- Ремедиация данных: отозвано 4 ложных сертификата и сброшено 3 ложных завершения (подпись — пустой `cmiData`), с бэкапом БД; двум реальным студентам отправлено письмо-объяснение. Диагностика: ложные completed = пустой CMI, реальные коммиты идут со статусом `incomplete`/`unknown`.
